...
def draw_lplot(ax, seifa_deciles, y, xlabel=None, ylabel=None, title=None, subplotlabel = None):
    deciles = [*range(1, 11)]
    ...
    for idx, col in enumerate(df_band_medians.columns[1:]):
        ax.errorbar(x=df_band_medians.index.values+1, y=df_band_medians[col], yerr = [np.abs(df_band_05[col].values-df_band_medians[col]), np.abs(df_band_95[col].values-df_band_medians[col])], 
